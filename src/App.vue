...
import { checkFloodHistory } from "./services/FloodHistory"; // FloodHistory
/* global kakao */
export default {
  data() {
    return {
  ...
  methods: {
    initializeMap(lat, lng) {
      ...
      kakao.maps.event.addListener(this.map, "click", async (mouseEvent) => {
        ...
        const floodOccurred = await checkFloodHistory(lat, lng, this.map);

        if (floodOccurred) {
          alert("해당 위치는 침수 이력이 있습니다.");
        } else {
          alert("해당 위치는 침수 이력이 없습니다.");
        }
      });
    },
    addMarker(lat, lng) {
